...
# 5. 윤곽선 탐지 및 가장 먼 점 두 개 탐색 (길이 측정)
...
plt.imshow(vis_img)
plt.title(f"🐟 Fish Length from TFLite Proto: {longest_dist:.2f} px")
plt.axis("off")
plt.tight_layout()
plt.savefig("fish_length_from_tflite_proto.png")

# Heatmap 저장
plt.figure(figsize=(6, 6))
plt.imshow(mask_resized, cmap='inferno')
plt.title("🔥 Proto-based Mask Heatmap (TFLite)")
plt.colorbar()
plt.tight_layout()
plt.savefig("mask_heatmap_from_tflite_proto.png")

print(f"✅ 시각화 완료! 길이: {longest_dist:.2f}px")
